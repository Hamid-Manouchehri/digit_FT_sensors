 ...
 * communication parameters, and continuously reads the sensor data while pushing
 * the stream to Lab Streaming Layer (LSL).
 *
 * License:
...
void readAndStreamFTData(std::string csv_file, CRT_RFT_UART& sensor, lsl::stream_outlet& outlet, int dataRateInterval) {
    ...
    while (true) {
        ...
        outlet.push_sample(FT_data);

        // Optionally, save the data to a CSV file
        SAVE_FT_TO_CSV_FILE(csv_file, FT_data, img_index);  
        img_index += 1;

        // Print the FT data to the console
        for (int i = 0; i < 6; ++i) {
            std::cout << FT_data[i];
            if (i < 5) std::cout << ", ";
        }
...
int main() {
    ...
    // Create the LSL stream outlet
    lsl::stream_info info("FT_Stream", "ForceTorque", 6, LSL_IRREGULAR_RATE, lsl::cf_float32);
    lsl::stream_outlet outlet(info);

    // Read and stream FT data continuously
    std::cout << "Starting to read and stream FT data..." << std::endl;
    readAndStreamFTData(full_path_to_FT_csv_file, RFT_SENSOR, outlet, dataRateInterval);
